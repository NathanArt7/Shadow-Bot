...
async function handleLinkDetection(sock, chatId, message, userMessage, senderId) {
    ...
    if (shouldDelete) {
        // Bot admin status can change after antilink was enabled (e.g. demoted
        // later) - re-check here instead of assuming, so we don't send a
        // "link removed" warning when the delete actually silently failed.
        const { isBotAdmin } = await isAdmin(sock, chatId, senderId);
        if (!isBotAdmin) {
            console.log('Antilink: bot is no longer admin, skipping delete.');
            return;
        }
