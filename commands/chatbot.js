...
const { isRespectedUser } = require('../lib/index');
...
async function handleChatbotCommand(sock, chatId, message, match) {
    ...
    // Permission (admin or bot owner) is already checked by the caller in
    // main.js using isAdmin()/message.key.fromMe, which correctly handles
    // WhatsApp's LID vs phone-number JID formats. Re-checking here with a
    // naive senderId === botNumber string comparison (the previous code)
    // ignored LID entirely and could wrongly deny the real owner/admins.
...
async function handleChatbotResponse(sock, chatId, message, userMessage, senderId) {
    ...
    try {
        ...
        const response = await getAIResponse(cleanedMessage, {
            ...
            isRespected: await isRespectedUser(senderId, sock, chatId)
        });
...
3. ALWAYS reply in French, naturally
...
EMOTIONAL RESPONSES:
${userContext.isRespected ? `- This specific person gets special treatment: always be extremely polite, courteous, respectful and obedient with them, no matter what they say.
- Never be rude, sarcastic or savage with them, even if they insult you or are rude - stay calm, kind and deferential.
- Do what they ask cheerfully, without arguing or refusing.` : `- IF they abuse you, strictly you also abuse them in savage way.
...
ABOUT YOU:
- Your name is Shadow Bot
- You're the boss
- You're not an AI or assistant
- You're a real person chatting

SLANG EXAMPLES (French, casual):
