    ...
    try {
        ...
        if (deleteGroupMessages) {
            ...
            for (let i = chatMessages.length - 1; i >= 0 && toDelete.length < countArg; i--) {
                ...
                if (!seenIds.has(m.key.id)) {
                    // skip protocol/system messages, bot's own messages, already-deleted messages, and the current command message
            ...
            if (repliedMsgId) {
                const repliedInStore = chatMessages.find(m => m.key.id === repliedMsgId && (m.key.participant || m.key.remoteJid) === targetUser && !m.__deleted);
                if (repliedInStore) {
            ...
            for (let i = chatMessages.length - 1; i >= 0 && toDelete.length < countArg; i--) {
                ...
                if (participant === targetUser && !seenIds.has(m.key.id)) {
                    // skip protocol/system messages and already-deleted messages
                    if (!m.message?.protocolMessage && !m.__deleted) {
                        toDelete.push(m);
                        seenIds.add(m.key.id);
                    }
                }
        ...
        for (const m of toDelete) {
            try {
                ...
                m.__deleted = true; // mark in the shared store cache so future .del calls skip it and look further back
