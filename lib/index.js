...
// Users the chatbot should always treat with extra courtesy/deference.
// Cross-references group metadata (id/lid/phoneNumber) instead of a bare string
// match, since WhatsApp may represent the same person differently (phone-number
// JID vs privacy LID) depending on context - same issue we hit with isAdmin.js.
async function isRespectedUser(userId, sock, chatId) {
    try {
        const data = loadUserGroupData();
        const list = Array.isArray(data.respectedUsers) ? data.respectedUsers : [];
        if (list.length === 0) return false;

        const normalize = (jid) => {
            if (!jid) return '';
            const noSession = jid.includes(':') ? jid.split(':')[0] + '@' + jid.split('@')[1] : jid;
            return noSession.split('@')[0];
        };

        const userNum = normalize(userId);
        if (list.some(j => normalize(j) === userNum)) return true;

        if (sock && chatId && chatId.endsWith('@g.us')) {
            try {
                const metadata = await sock.groupMetadata(chatId);
                const participant = metadata.participants.find(p => {
                    return normalize(p.id) === userNum || normalize(p.lid) === userNum || normalize(p.phoneNumber) === userNum;
                });
                if (participant) {
                    const candidates = [participant.id, participant.lid, participant.phoneNumber]
                        .map(normalize)
                        .filter(Boolean);
                    if (list.some(j => candidates.includes(normalize(j)))) return true;
                }
            } catch (e) {}
        }
        ...
        console.error('Error checking respected user:', error);
        return false;
    }
}

async function addRespectedUser(userJid) {
    try {
        const data = loadUserGroupData();
        if (!data.respectedUsers) data.respectedUsers = [];
        if (!data.respectedUsers.includes(userJid)) {
            data.respectedUsers.push(userJid);
            saveUserGroupData(data);
        }
        return true;
    } catch (error) {
        console.error('Error adding respected user:', error);
        return false;
    }
}

async function removeRespectedUser(userJid) {
    try {
        const data = loadUserGroupData();
        if (!data.respectedUsers) data.respectedUsers = [];
        const idx = data.respectedUsers.indexOf(userJid);
        if (idx !== -1) {
            data.respectedUsers.splice(idx, 1);
            saveUserGroupData(data);
        }
        return true;
    } catch (error) {
        console.error('Error removing respected user:', error);
        return false;
    }
}

async function getRespectedUsers() {
    try {
        const data = loadUserGroupData();
        return Array.isArray(data.respectedUsers) ? data.respectedUsers : [];
    } catch (error) {
        console.error('Error getting respected users:', error);
        return [];
    }
}
...
async function setGroupActive(groupId, enabled) {
    try {
        const data = loadUserGroupData();
        if (!data.activeGroups) data.activeGroups = {};
        data.activeGroups[groupId] = enabled;
        saveUserGroupData(data);
        return true;
    } catch (error) {
        console.error('Error setting group active state:', error);
        return false;
    }
}
...
async function isGroupActive(groupId) {
    try {
        const data = loadUserGroupData();
        // Disabled by default: a group must be explicitly activated.
        return !!(data.activeGroups && data.activeGroups[groupId]);
    } catch (error) {
        console.error('Error checking group active state:', error);
        return false;
    }
}
