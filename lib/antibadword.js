...
async function handleBadwordDetection(sock, chatId, message, userMessage, senderId) {
    ...
    const cleanMessage = userMessage.toLowerCase()
        .normalize('NFD').replace(/[̀-ͯ]/g, '') // Strip accents (é -> e, â -> a...) so they don't get cut into spaces below
    ...
    const badWords = [
        ...
        // French
        'putain', 'merde', 'connard', 'connasse', 'salope', 'pute', 'enculé', 'enculer',
        'encule', 'con', 'conne', 'batard', 'bâtard', 'fdp', 'ntm', 'nique ta mère',
        'nique ta mere', 'niquer', 'bite', 'couille', 'couilles', 'chier', 'bordel',
        'branler', 'branleur', 'branleuse', 'salaud', 'pd', 'pédé', 'pede', 'tarlouze',
        'pétasse', 'petasse', 'garce', 'enfoiré', 'enfoire', 'fils de pute', 'ta gueule',
        'tg', 'abruti', 'debile', 'débile', 'imbécile', 'imbecile', 'crétin', 'cretin',
        'connard de merde', 'grosse pute', 'sale pute', 'sale con', 'petit con'
    ];
   ...
   // console.log('Bad word detected in:', userMessage);
    ...
    // Check if bot and sender are admin (reuses the robust LID/phone-number matching
    // in isAdmin.js instead of a naive p.id === botId check, which silently fails
    // whenever WhatsApp lists a participant by LID instead of phone number).
