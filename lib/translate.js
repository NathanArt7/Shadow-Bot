...
async function translateText(text, lang, sourceLang = 'auto') {
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.error('[translate] dreaded.site: unexpected response', JSON.stringify(data).slice(0, 200));
        } else {
            console.error(`[translate] dreaded.site: HTTP ${response.status} ${response.statusText}`);
        }
    } catch (e) {
        console.error('[translate] dreaded.site error:', e.message);
    }
