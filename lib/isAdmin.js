...
async function isAdmin(sock, chatId, senderId) {
    try {
        ...
        if (!isBotAdmin) {
            console.log('[DEBUG isAdmin] botId:', botId, '| botLid:', botLid);
            console.log('[DEBUG isAdmin] participants:', JSON.stringify(participants.map(p => ({ id: p.id, lid: p.lid, admin: p.admin })), null, 2));
        }
