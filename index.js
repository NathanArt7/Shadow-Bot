...
async function startXeonBotInc() {
    ...
    XeonBotInc.ev.on('connection.update', async (s) => {
        ...
        if (connection == "open") {
            ...
            console.log(chalk.magenta(`\n${global.themeemoji || '•'}⚡ SHADOW BOT ACTIVE`))
            console.log(chalk.magenta(`${global.themeemoji || '•'} GITHUB: NathanArt7`))
            console.log(chalk.magenta(`${global.themeemoji || '•'} WA NUMBER: ${owner}`))
            console.log(chalk.green(`${global.themeemoji || '•'} 🤖 Bot Connected Successfully! ✅`))
            console.log(chalk.blue(`Bot Version: ${settings.version}`));

            // Best-effort: cache each group's name + invite code (if admin there),
            // so a rejoin can be attempted if the bot is later removed.
            (async () => {
                try {
                    const groups = await XeonBotInc.groupFetchAllParticipating();
                    for (const groupId of Object.keys(groups)) {
                        await refreshGroupCache(XeonBotInc, groupId);
                    }
                } catch (e) {
                    console.error('Failed to warm up group invite cache:', e.message);
                }
            })();
        }
